...
public:
  ...
  SYSTEM_ERROR2_CONSTEXPR14 errored_status_code clone() const { return *this; }
...
public:
  ...
  SYSTEM_ERROR2_CONSTEXPR20 errored_status_code clone() const
  {
    ...
    if(errcode != 0)
    {
      SYSTEM_ERROR2_FATAL("errored_status_code<erased>::clone() failed");
    }
    return x;
  }

  //! Permit casting to `status_code`
  SYSTEM_ERROR2_CONSTEXPR14 operator const status_code<detail::erased<ErasedType>> &() const &
  {
  ...
  //! Permit casting to `status_code`
  SYSTEM_ERROR2_CONSTEXPR14 operator status_code<detail::erased<ErasedType>> &&() &&
  {
